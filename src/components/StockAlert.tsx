...
function StockAlertPopup({ 
  ...
  const popup = (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 animate-fadeIn">
      ...
      {/* 팝업 모달 */}
      <div className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-3xl shadow-2xl shadow-red-500/20 border border-red-500/30 max-w-lg w-full max-h-[80vh] overflow-hidden animate-slideDown">
        {/* 상단 빨간 경고 바 */}
              ...
              <div className="space-y-2">
                {urgentItems.slice(0, 5).map((item, idx) => (
                  <div 
                    ...
                    className="p-3 bg-red-500/10 border border-red-500/30 rounded-xl cursor-pointer hover:bg-red-500/20 transition-all"
                  >
                    <div className="flex items-center justify-between">
                      <div>
                      ...
                      <div className="text-right">
                        <p className="text-red-400 font-bold">{item.currentStock} / {item.optimalStock}</p>
                        <p className="text-xs text-red-300">{item.shortage}개 부족</p>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-2">
                {warningItems.slice(0, 5).map((item, idx) => (
                  <div 
                    key={item.id || idx}
                    onClick={() => onItemClick?.(item)}
                    className="p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-xl cursor-pointer hover:bg-yellow-500/20 transition-all"
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-white font-medium">{item.itemName}</p>
                      ...
                      <div className="text-right">
                        <p className="text-yellow-400 font-bold">{item.currentStock} / {item.optimalStock}</p>
                        <p className="text-xs text-yellow-300">{item.shortage}개 부족</p>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="p-4 border-t border-slate-700 flex gap-3">
          ...
          <button
            ...
            className="flex-1 py-3 bg-gradient-to-r from-red-600 to-orange-500 hover:from-red-500 hover:to-orange-400 text-white font-bold rounded-xl transition-all"
          >
            지금 확인하기
          </button>
        </div>
...
export default function StockAlert({
  ...
  if (error && !result) {
    return (
      <>
        {alertPopup}
        {topBanner}
        <div className="bg-slate-800/50 rounded-2xl border border-slate-600/30 p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-slate-300 font-medium">재고 체크</p>
                <p className="text-xs text-slate-500">{error}</p>
              </div>
            </div>
            <button onClick={checkStock} disabled={isChecking} className="px-3 py-1.5 bg-slate-600/30 hover:bg-slate-600 text-slate-300 text-sm rounded-lg transition-all disabled:opacity-50">
  ...
  if (!hasLowStock && result) {
    return (
      <>
        {alertPopup}
        {topBanner}
        <div className="bg-gradient-to-br from-emerald-900/30 to-green-900/20 rounded-2xl border border-emerald-500/30 p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-emerald-400 font-medium">재고 상태 양호</p>
            ...
            <button onClick={checkStock} disabled={isChecking} className="px-3 py-1.5 bg-emerald-600/30 hover:bg-emerald-600 text-emerald-300 text-sm rounded-lg transition-all disabled:opacity-50">
        ...
        <div className="bg-slate-800/50 rounded-2xl border border-slate-600/30 p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-slate-300 font-medium">재고 부족 체크</p>
                <p className="text-xs text-slate-500">
                  {isChecking ? 'AI가 재고를 분석 중입니다...' : '현재재고 vs 적정재고 비교'}
                </p>
              </div>
            </div>
            <button onClick={checkStock} disabled={isChecking} className="px-3 py-1.5 bg-violet-600/30 hover:bg-violet-600 text-violet-300 text-sm rounded-lg transition-all disabled:opacity-50">
  ...
  return (
    <>
      {alertPopup}
      {topBanner}
      <div className="bg-gradient-to-br from-red-900/30 to-orange-900/20 rounded-2xl border border-red-500/30 overflow-hidden">
      {/* 헤더 */}
      <div className="flex items-center justify-between p-4 cursor-pointer hover:bg-red-500/10 transition-colors" onClick={() => setIsExpanded(!isExpanded)}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-white">재고 부족 알림</h3>
            <p className="text-xs text-gray-400">{lowStockCount}개 품목이 적정 재고 미만입니다</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={(e) => { e.stopPropagation(); checkStock(); }} disabled={isChecking} className="p-2 bg-orange-600/30 hover:bg-orange-600 text-orange-300 rounded-lg transition-all disabled:opacity-50">
        ...
        <div className="px-4 pb-3">
          <div className="p-3 bg-violet-500/10 border border-violet-500/30 rounded-xl">
            <div className="flex items-start gap-2">
              ...
              <p className="text-sm text-gray-200 leading-relaxed whitespace-pre-wrap">{aiSummary}</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="px-4 pb-4">
          <div className="space-y-2 max-h-80 overflow-y-auto">
            {lowStockItems.map((item, idx) => (
              <div key={item.id || idx} onClick={() => onAlertClick?.(item)} className={`p-3 rounded-xl border cursor-pointer hover:scale-[1.02] transition-all ${getUrgencyColor(item.shortagePercent)}`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className={`px-2 py-0.5 rounded text-xs font-bold ${item.shortagePercent >= 70 ? 'bg-red-600 text-white' : item.shortagePercent >= 50 ? 'bg-orange-600 text-white' : 'bg-yellow-600 text-white'}`}>
                  ...
                  <div className="text-right">
                    <p className="font-mono">
                      ...
                      <span className="text-gray-300">{item.optimalStock}</span>
                    </p>
          ...
          <div className="mt-4 pt-3 border-t border-red-500/20 text-xs text-gray-500 flex justify-between">
